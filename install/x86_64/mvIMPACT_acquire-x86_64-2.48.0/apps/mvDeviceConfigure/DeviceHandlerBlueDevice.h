...
protected:
    void                                CloseAndReOpenInGenICamInterfaceLayout( void ) const;
    ...
    void                                SelectCustomGenICamFile( const wxString& descriptionFile = wxEmptyString ) const;
...
public:
    ...
    const wxString&                     GetCustomGenICamFile( void ) const
    {
        return GenICamFile_;
    }
    void                                SetCustomGenICamFile( const wxString& customGenICamFile )
    {
...
private:
    ...
    unsigned int                        CountWildcards( const wxString& stringToEvaluate ) const;
    wxArrayString::const_iterator       GetBestMatchingFirmwareChoice( wxArrayString& choices, bool& boMultipleBestMatchesFound );
    ...
    std::vector<std::map<wxString, SuitableProductKey>::const_iterator> GetCompatibleIteratorsForProduct( const FileEntry& entry, const wxString& product, const wxString& productFromManufacturerInfo ) const;
    ...
    bool                                VerifyFirmwareChecksum( bool boSilentMode, const wxString& serial, auto_array_ptr<char>& pArchive, const wxString& firmwareArchivePath, const wxString& firmwareFileName );
protected:
    enum TProductGroup
    {
        pgUnknown,
    ...
    static bool                         ManufacturerStringUsesMVFormat( const wxString& manufacturerSpecificInformation, wxArrayString* pTokens = 0 )
    {
    ...
    static bool IsSpecificType( const wxString& manufacturerSpecificInformation, const wxString& typeSuffix )
    {
        ...
        if( ManufacturerStringUsesMVFormat( manufacturerSpecificInformation, &tokens ) )
        {
            return tokens[3].find( typeSuffix ) == 0;
        }
        return false;
    }
...
public:
    ...
    {
        if( ConvertedString( pDev->family.read() ).Find( wxT( "mvBlueFOX3" ) ) != wxNOT_FOUND )
        {
            return true;
        }
        const wxString token = GetProductFromManufacturerInfo( pDev );
        const std::string tokenANSI( token.mb_str() );
        if( token.StartsWith( wxT( "SF" ) ) )
        {
            return true;
        }

        if( ( tokenANSI.length() > 3 ) &&
            ( token[0] == '2' ) &&
            isdigit( token[1] ) &&
            isdigit( token[2] ) &&
            isdigit( token[3] ) )
    ...
    virtual bool                        SupportsFirmwareVerification( void ) const
    {
    ...
    virtual int                         VerifyFirmware( bool boSilentMode );
};
